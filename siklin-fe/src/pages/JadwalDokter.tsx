...
const JadwalDokter = () => {
  ...
  const userRole = typeof user?.role === "string" 
    ...
    : (user?.role as any)?.name?.toUpperCase() || (user?.role as any)?.nama?.toUpperCase() || "";

  const hasManagePermission = Array.isArray(user?.permissions) && user.permissions.includes("JADWAL_MANAGE");
  const canManageJadwal = userRole === "SUPERADMIN" || userRole === "DOKTER" || hasManagePermission;
  ...
  // Logika untuk mengurutkan UUID dokter menjadi Dokter 1, Dokter 2, dst.
  const uniqueDoctors = Array.from(new Set(jadwal.map((item) => item.dokterId).filter(Boolean)));
      ...
      {canManageJadwal && (
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <form onSubmit={handleSimpan} className="flex flex-wrap gap-4 items-end">
            ...
            <div className="flex-1 min-w-[150px]">
              <input type="text" placeholder="ID Dokter (UUID)" value={dokterId} onChange={(e) => setDokterId(e.target.value)} required className="w-full rounded-xl border border-slate-300 p-2.5 text-sm outline-none focus:border-cyan-500" />
            </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {jadwal.length === 0 ? (
            ...
            jadwal.map((item: any) => {
              // 1. Coba ambil nama asli dari backend (jika relasi sudah jalan di API)
              let namaDokter = item.dokter?.nama || item.dokter?.name || item.user?.nama || item.user?.name;
              
              // 2. Jika nama asli kosong, kita ubah UUID panjang menjadi alias "Dokter 1", "Dokter 2", dst.
              if (!namaDokter) {
                const docIndex = uniqueDoctors.indexOf(item.dokterId) + 1;
                namaDokter = `Dokter ${docIndex}`;
              }
